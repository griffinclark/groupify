...
export const LocationDetails: React.FC<Props> =   ({ location, userLocation, closeLocationDetail }: Props) => {
    ...
    useEffect(() => {
      (async () => {
        setDuration(await durationCalculation({lat: userLocation.latitude, lng: userLocation.longitude}, location.geometry.location));
    ...
    return (
        <View style={styles.locationDetailsContainer}>
            ...
            {location.photos &&
                (<ActivityImage containerStyle={styles.paddingBottomTen} referenceId={location.photos[0].photo_reference} height={150} width={Dimensions.get("window").width - 40} />)
            }

            <LocationRating containerStyle={styles.paddingBottomTen} rating={location.rating} ratingTotal={location.user_ratings_total} />
